...
def plot_real_vs_pred(system,target,X_test,y_test,reg_ln,reg_KN,reg_DT,reg_XGB,reg_LGBM):
  ...
  y_real.rename(columns={target:'Pred'},inplace=True)
  y_real['Model']='Real'
  y_real.reset_index(inplace=True)

  y_pred_ln=pd.DataFrame(reg_ln.predict(X_test[X_test[system]==1]),columns=['Pred'])
  y_pred_ln['Model']='Linear Regresor (Base model)'

  y_pred_KN=pd.DataFrame(reg_KN.predict(X_test[X_test[system]==1]),columns=['Pred'])
  y_pred_KN['Model']='KNeighbors'

  y_pred_DT=pd.DataFrame(reg_DT.predict(X_test[X_test[system]==1]),columns=['Pred'])
  y_pred_DT['Model']='Decision Tree'

  y_pred_XGB=pd.DataFrame(reg_XGB.predict(X_test[X_test[system]==1]),columns=['Pred'])
  y_pred_XGB['Model']='XGBoost'

  y_pred_LGBM=pd.DataFrame(reg_LGBM.predict(X_test[X_test[system]==1]),columns=['Pred'])
  ...
  sns.lineplot(data=result[models_filter0],y='Pred',x=result[models_filter0].index,hue='Model',ax=ax[0])
  sns.lineplot(data=result[models_filter1],y='Pred',x=result[models_filter1].index,hue='Model',ax=ax[1])
  sns.lineplot(data=result[models_filter2],y='Pred',x=result[models_filter2].index,hue='Model',ax=ax[2])
  sns.lineplot(data=result[models_filter3],y='Pred',x=result[models_filter3].index,hue='Model',ax=ax[3])
